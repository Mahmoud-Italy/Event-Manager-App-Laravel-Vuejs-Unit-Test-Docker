              ...
              <div class="row" v-if="!pgLoading && !something_went_wrong">
                    ...
                    <div class="card">
                      <div class="card-body">
                          <div class="form-group">
                            <label for="txt1">{{ $t('meetings.title') }}</label>
                            <input type="text" class="form-control" id="txt1" v-model="row.title" required="">
                          </div>
                          <div class="form-group">
                            <label for="txt2">{{ $t('meetings.location') }}</label>
                            <input type="text" class="form-control" id="txt2" v-model="row.location" required="">
                          </div>
                        <div class="row col-12">
                          <div class="form-group col-6">
                            <label for="txt3">{{ $t('meetings.start_dateTime') }}</label>
                            <date-picker v-model="row.start_dateTime" id="txt3" type="datetime" 
                                  style='width: 100%; height: 35px'></date-picker>
                          </div>
                          <div class="form-group col-6">
                            <label for="txt4">{{ $t('meetings.end_dateTime') }}</label>
                ...
                <div class="col-md-8 grid-margin stretch-card">
                  <div class="card">
                    <div class="card-body dir-ltr">
                       <h6 class="card-title">{{ $t('meetings.content') }}</h6>
                ...
                <div class="col-md-4 grid-margin stretch-card">
                  <div class="card" style="height: 350px">
                    <div class="card-body">
                      <h6 class="card-title">{{ $t('meetings.addMember') }} ( {{ total_data }} / 4 )</h6>
                        <Dropdown v-if="total_data < 4"
                          :options="members"
  ...
  export default {
    ...
    methods: {
        fetchData(loader = true) {
          ...
          formData.append('meeting_id', this.$route.params.id);
          formData.append('accessToken', this.accessToken);
          axios.post('/api/v1/meeting/edit', formData, config)
            .then(res => {
              this.row.image = res.data.row.image;
              this.row.title = res.data.row.title;
              this.row.content = res.data.row.content;
              this.row.location = res.data.row.location;
        ...
        validateSelection(e) {
            ...
            formData.append('meeting_id', this.$route.params.id);
            formData.append('accessToken', this.accessToken);
            axios.post('/api/v1/meeting/invite/member', formData, config)
            .then(res => {
        ...
        deleteData(id) {
            ...
            formData.append('meeting_id', this.$route.params.id);
            formData.append('accessToken', this.accessToken);
            axios.post('/api/v1/meeting/remove/member', formData, config)
            .then(res => {
              this.fetchData(false);
              this.tblLoading = false;
            })

            ...
            <div class="page-content">
              <nav class="page-breadcrumb">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">{{ $t('meetings.name') }}</li>
                </ol>
              </nav>
                  ...
                  <div class="card">
                    <div class="card-body">
                      <h6 class="card-title fontana">{{ $t('app.all_data') }} ( {{ total_data }} )
                        <p class="text-right btn-header">
                          <router-link :to="{ name:'meeting-create' }" class="btn btn-primary pull-right">{{ $t('app.create_new') }}</router-link>
                        </p>
                      </h6>
                          ...
                          <table class="table table-hover">
                            <thead>
                              <tr>
                                ...
                                <th class="text-center" style="width:10%">{{ $t('meetings.image') }}</th>
                                <th class="text-center" style="width:15%">{{ $t('meetings.title') }}</th>
                                <th class="text-center" style="width:15%">{{ $t('meetings.location') }}</th>
                                <th class="text-center" style="width:15%">{{ $t('meetings.members') }}</th>
                                <th class="text-center" style="width:10%">{{ $t('meetings.start_dateTime') }}</th>
                                <th class="text-center" style="width:10%">{{ $t('meetings.end_dateTime') }}</th>
                            ...
                            <tbody v-if="!dataLoading">
                              ...
                              <tr v-if="total_data != 0" v-for="(row, index) in rows" :key="index" v-bind:key="row.id" :ref="'row_'+row.id"
                                  ...
                                  <td class="text-center">
                                    <img v-if="row.image !== null" :src="row.image" v-bind:style="{'width':'50px','height':'50px', 'border-radius':'0'}">
                                  </td>
                                  <td class="text-center">{{ row.title }}</td>
                                  <td class="text-center">{{ row.location }}</td>
                                  <td class="text-center">{{ row.total_members }} / 4</td>
                                  <td class="text-center">{{ row.start_date }} <p>{{ row.start_time }}</p></td>
                                  <td class="text-center">{{ row.end_date }} <p>{{ row.end_time }}</p></td>
                                  <td class="text-center">
                                      ...
                                      <button type="button" class="btn btn-success"
                                              @click="row.edit_loading = true; editData(row.id)" :disabled="row.edit_loading">
                                            <span v-if="!row.edit_loading">{{ $t('app.edit') }}</span>
  ...
  export default {
    ...
    methods: {
      fetchData(page_url) {
          ...
          page_url = page_url || '/api/v1/meetings';
        ...
        activeData(id) {
            ...
            formData.append('modelName', 'Meeting');
            formData.append('accessToken', this.accessToken);
        ...
        editData(id) {
          this.$router.push({ name: 'meeting-edit', params: {id:id} })
        },
        deleteData(id) {
          if(confirm('Are You Sure?')) {
            ...
            axios.post('/api/v1/meeting/destroy', formData, config)
                .then(res => {
                this.fetchData();
                this.$noty.info(this.$i18n.messages[this.$i18n.locale].app.success_msg);
             })
